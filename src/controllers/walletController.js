...
console.log(`🔧 Wallet Controller: ${USE_DEMO_MODE ? 'DEMO' : 'REAL'} mode`);

exports.getBalance = async (req, res) => {
  try {
    const companyId = req.user?.companyId || 2;
    const balance = await walletService.getBalance(companyId);
    const availableCalls = await walletService.getAvailableCallCount(companyId);
...
exports.initiateTopup = async (req, res) => {
  try {
    ...
    console.log('💰 Initiating topup:', { companyId, amount, mode: USE_DEMO_MODE ? 'DEMO' : 'REAL' });
    ...
    // ===== DEMO MODE =====
    if (USE_DEMO_MODE) {
      console.log('🎭 DEMO MODE: Adding balance directly');
      ...
      console.log('✅ Demo balance added:', result);

      return res.json({
        success: true,
        paymentID: `DEMO-${Date.now()}`,
        bkashURL: null,
        message: `✅ Demo payment successful! ${amount} TK added to your wallet.`,
        demoMode: true,
        amount: amount,
...
exports.bkashCallback = async (req, res) => {
  try {
    ...
    if (USE_DEMO_MODE) {
      console.log('🎭 Demo mode callback - skipping');
      const frontendURL = process.env.FRONTEND_URL || 'http://localhost:3000';
      return res.redirect(`${frontendURL}/credit?success=true&amount=50`);
    }
...
// ===== ADD TEST BALANCE (Extra) =====
exports.addTestBalance = async (req, res) => {
  try {
    const { amount, companyId } = req.body;
    const cId = companyId || 2;
    const amt = amount || 100;
    
    console.log('🧪 Adding test balance:', { companyId: cId, amount: amt });
    ...
    res.json({
      success: true,
      message: `Test balance of ${amt} TK added successfully`,
      newBalance: result.newBalance
    });
  } catch (error) {
